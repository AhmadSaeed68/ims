...
class Sale_order_modal extends CI_Model
{
   ...
   function sales_order($user_id) {
           $data=$this->db
            ...
            function get_item_code_in_so($user_id){
                  $asset=$this->db
                  ->select(['item_code'])
                  ->group_by('item_code')
                  ->from('items_in_stock')
              ...
              // function get_invo_in_so($item_code, $user_id){
              //     $asset=$this->db
              //     ->select(['invoice_code'])
              //     ->from('items_in_stock')
              //     ->where('user_id',$user_id)
              ...
              function get_users_in_so($user_id)
              {
                $data=$this->db
              ...
              function city_on_change($business_name_id,$user_id){
                $data=$this->db
                                ->select('*')
                                ->from('user_inform')
              ...
              function view_so($so_code,$user_id){

                  $data=$this->db
                    ->query('SELECT sale_order.so_code,sale_order.invoice_code,sale_order.customer_name,sale_order_detail.item_code,sale_order_detail.item_qty,sale_order_detail.item_rate,sale_order_detail.date,sale_order_detail.so_item_total,sale_order_detail.profit FROM sale_order_detail  LEFT JOIN sale_order ON sale_order_detail.so_code=sale_order.so_code WHERE sale_order.so_code="'.$so_code.'" and sale_order.user_id="'.$user_id.'" and sale_order_detail.user_id="'.$user_id.'"');
              return $data->result_array();
              }

              function edit_so($so_id,$user_id){
                  $data=$this->db
                    ->query('SELECT sale_order_detail.id,sale_order_detail.date,sale_order.so_status, sale_order_detail.profit,sale_order_detail.so_code,sale_order_detail.item_code,sale_order_detail.item_qty,sale_order_detail.item_rate,sale_order_detail.so_item_total,sale_order_detail.invoice_code,sale_order.so_code,sale_order.customer_name FROM sale_order_detail LEFT JOIN sale_order ON sale_order.so_code=sale_order_detail.so_code where sale_order.user_id="'.$user_id.'" and sale_order_detail.user_id="'.$user_id.'"');
                return $data->result_array();
              }
              ...
              function item_data($item_code,$user_id)
              {
              ...
              function so_status($status,$user_id){
                      $so_code= $this->input->post('so_code');
                      $status;
